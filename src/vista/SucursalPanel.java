...
public class SucursalPanel extends JPanel {
    ...
    private void initUI() {
        ...
        btnAgregarEmpleado.addActionListener(e -> {
            ...
            Sucursal s = controlador.getSucursales().get(idx);
    ...
    private void cargarSucursales() {
        modelSuc.clear();
        List<Sucursal> suc = controlador.getSucursales();
        for (Sucursal s : suc) {
            modelSuc.addElement(s.getCodigo() + " - " + s.getDireccion() + " (empleados: " + s.getEmpleados().size() + ")");
        }
    }
}
